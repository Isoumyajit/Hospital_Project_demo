...
session_start();
 ...
 if(isset($_POST['add-staff']))
 {
     ...
     $desig = $_POST['desig'];
     $salary = $_POST['salary'];
     $query = "INSERT INTO STAFF(SID,NAME,ADDRESS,EMAIL,PHONE,DEPT,DESIGNATION,SALARY) VALUES ('$sid','$name','$address','$email','$phone','$dept','$desig','$salary')";
     $conn->query($query);
     if($conn->affected_rows > 0)
      {
...
if(isset($_GET['delete-staff'])){
 ...
 if(isset($_GET['edit-staff'])){
    ...
    //This is for the doctor-date page
...
if(!empty($_POST["deptname"])){ 
    ...
    $name = $_POST['deptname'];
    echo "$name";
    $query = "SELECT NAME FROM  DOCTOR  WHERE DEPT = '$name' "; 
...
if(isset($_POST['add-time'])){
        ...
        $dept = $_POST['dept'];
        $day = $_POST['day'];
        $dayarray = implode(',',$day);
        $time = $_POST['time'];
        $did = $_POST['Doctor_id'];
        $visit = $_POST['visiting'];
        echo $visit;
        $query = "INSERT INTO doctor_time(DID,dept,doctor,visiting,days,time) VALUES ('$did','$dept','$doctor_name',$visit,'$dayarray','$time')";
        $conn->query($query);
        if($conn->affected_rows > 0 )
        {
             $del = "DELETE FROM rating WHERE DID = '$did'";
             $conn->query($del);
            header('location:doctor-date.php');
    ...
    if(isset($_GET['delete-time'])){
        echo $_GET['dept'];
          $id = $_GET['delete-time'];
           $dept = $_GET['dept'];
          $query = "DELETE FROM doctor_time WHERE doctor = '$id' AND dept='$dept'";
          $conn->query($query);
          if($conn->affected_rows > 0 )
          {
              header('location:doctor-date.php');
      ...
      if(isset($_GET['approve-salary'])){
        echo "if block";
       $id = $_GET['approve-salary'];
      $delete = "DELETE FROM salary_manage WHERE SID = '$id'";
      $conn->query($delete);
      if($conn->affected_rows > 0)
         {
             header('location:salary.php');
             $_SESSION['response'] = "Successfully Approved";
